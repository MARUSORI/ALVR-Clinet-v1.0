...
//            info.flags=201203;
//            info.FrameIndex = i++;
//            LOG("jay room set up!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d",info.flags);
//            this->send(&info, sizeof(info));
//            //roomsetup = false;
//        }
        // endof jay fix
...
void UdpManager::processVideoSequence(uint32_t sequence) {
    if (m_prevVideoSequence != 0 && m_prevVideoSequence + 1 != sequence) {
        ...
        LOGE("jay VideoPacket loss %d (%d -> %d)", lost, m_prevVideoSequence + 1,
             sequence - 1);
    }
...
void UdpManager::onPacketRecv(const char *packet, size_t packetSize) {
    ...
    if (type == ALVR_PACKET_TYPE_VIDEO_FRAME) {
        ...
        LOG("jay Received ALVR_PACKET_TYPE_VIDEO_FRAME!!!");
        ...
        // processPacket
        LOG("jay m_nalParser->processPacket!!!");
...
extern "C"
JNIEXPORT void JNICALL
Java_com_polygraphene_alvr_UdpReceiverThread_SendTrackingInfoNative(JNIEnv *env, jobject instance,
                                                                    jlong udpManager,
    ...
    if(position_ != NULL && orientation_ != NULL) {
        ...
        // jay fix
        g_udpManager->fetchTrackingInfo(env, (UdpManager *)udpManager, &position, &orientation);
    }
    else{
        g_udpManager->fetchTrackingInfo(env, (UdpManager *)udpManager, NULL, NULL);
    }
}

// Called TrackingThread. So, we can't use this->env.
void UdpManager::fetchTrackingInfo(JNIEnv *env_, UdpManager *udpManager, ovrVector3f *position,
                                  ovrQuatf *orientation) {
...
//        quat.w = sqrtf(0.5);
        ...
        //LOG("jay Position!=NULL %f,%f,%f,%f", orientation->x, orientation->y,orientation->z,orientation->w);

        ovrQuatf orientation_rotated = quatMultipy(orientation, &quat);

        orientation_rotated.x= orientation->x;
        orientation_rotated.y= orientation->y;
        orientation_rotated.z= orientation->z;
        orientation_rotated.w= orientation->w;

        //LOG("jay Position!=NULL %f,%f,%f,%f", orientation_rotated.x, orientation_rotated.y,orientation_rotated.z,orientation_rotated.w);

        frame->tracking.HeadPose.Pose.Orientation = orientation_rotated;

        position->y += position_offset_y; // 有点问题
        //sendTrackingInfo(&info, frame->displayTime, &frame->tracking, position,&orientation_rotated);
        sendTrackingInfo(&info, frame->displayTime, &frame->tracking,NULL,NULL);
    } else {
        // Non AR
        LOG("jay Position Non AR");
        sendTrackingInfo(&info, frame->displayTime, &frame->tracking, NULL, NULL);
    }
...
void UdpManager::sendTrackingInfo(TrackingInfo *packet, double displayTime, ovrTracking2 *tracking,
