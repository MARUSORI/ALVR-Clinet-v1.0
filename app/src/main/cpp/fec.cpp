...
void FECQueue::addVideoPacket(const VideoFrame *packet, int packetSize, bool &fecFailure) {
    LOG("jay addVideoPacket, m_recovered:%d",m_recovered);
    if (m_recovered && m_currentFrame.frameIndex == packet->frameIndex) {
        LOG("jay addVideoPacket return directly!!!");
        return;
    }
    // 检查是否是新的一帧!
    if (m_currentFrame.frameIndex != packet->frameIndex) {
        ...
        if (!m_recovered) {
            FrameLog(m_currentFrame.frameIndex,
                     "jay addVideoPacket::Previous frame cannot be recovered. shards=%u:%u frameByteSize=%d fecPercentage=%d m_totalShards=%u m_shardPackets=%u m_blockSize=%u",
                     m_totalDataShards,
                     m_totalParityShards,
            ...
            for (int packet = 0; packet < m_shardPackets; packet++) {
                FrameLog(m_currentFrame.frameIndex,
                         "jay addVideoPacket::packetIndex=%d, shards=%u:%u",
        ...
        FrameLog(m_currentFrame.frameIndex,
                 "jay addVideoPacket::Start new frame. frameByteSize=%d fecPercentage=%d m_totalDataShards=%u m_totalParityShards=%u m_totalShards=%u m_shardPackets=%u m_blockSize=%u",
    ...
    if (m_marks[packetIndex][shardIndex] == 0) {
        // Duplicate packet.
        LOGI("jay Packet duplication. packetCounter=%d fecIndex=%d", packet->packetCounter,
             packet->fecIndex);
        return;
    }
    ...
    //LOGI("jay Ready to Call memcpy payload");
    ...
    //LOGI("jay end of Call memcpy payload");
}

bool FECQueue::reconstruct() {
    LOGI("jay enter FECQueue::reconstruct");
    if (m_recovered) {
        LOGI("jay retrun reconstruct %",m_recovered);
        return false;
    }
    ...
    for (int packet = 0; packet < m_shardPackets; packet++) {
        ...
        if (m_receivedDataShards[packet] == m_totalDataShards) {
            ...
            FrameLog(m_currentFrame.frameIndex, "jay No need for FEC. packetIndex=%d", packet);
        ...
        if (m_rs->shards < m_totalDataShards) {
            ...
            LOGI("jay FECQueue::reconstruct Not enough parity data");
            ret = false;
            continue;
        }

        FrameLog(m_currentFrame.frameIndex,
                 "jay Recovering. packetIndex=%d receivedDataShards=%d/%d receivedParityShards=%d/%d",
        ...
        if (result != 0) {
            LOGE("jay reed_solomon_reconstruct failed.");
            return false;
        }
        /*
    ...
    if (ret) {
        m_recovered = true;
        FrameLog(m_currentFrame.frameIndex, "jay Frame was successfully recovered by FEC.");
    }
    return ret;
}
