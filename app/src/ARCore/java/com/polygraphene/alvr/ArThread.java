...
public class ArThread extends ThreadBase {
    private static final String TAG = "jay ArThread";
    ...
    ArThread(EGLContext EGLContext) {
        Log.d(TAG, "ArThread: Create");
        mEGLContext = EGLContext;
    }

    public void start() {
        if (mSession == null) {
            Log.d(TAG, "start: mSession == null");
            return;
        }
    ...
    public void initialize(MainActivity activity) {
        try {
            Log.d(TAG, "ArThread: ready to initialize session !");
            ...
            if (session == null) {
                ...
                Log.d(TAG, "Please install ARCore apk from store.");
                return;
            }
            Log.d(TAG, "ArThread: session createArSession successful!!!");
            mSession = session;
    ...
    private Session createArSession(MainActivity activity, boolean installRequested)
        ...
        if (hasCameraPermission(activity)) {
            ...
            Log.d(TAG, "createArSession: hasCameraPermission");
